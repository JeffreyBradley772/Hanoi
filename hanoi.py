...
class HanoiTowers(object):
   ...
   def __init__(self, size):
      ...
      self.tower1 = LinkedStack()
      self.tower2 = LinkedStack()
      self.tower3 = LinkedStack()
   ...
   def makeMove(self, source, destination):
      ...
      if self.towers[source].isEmpty():
         return
      if self.towers[destination].isEmpty():
         x = self.towers[source].pop()
         self.towers[destination].push(x)
         self.numMoves +=1
         
      elif self.towers[source].peek() < self.towers[destination].peek():
         x = self.towers[source].pop()
         self.towers[destination].push(x)
         self.numMoves += 1
   ...
   def detectWin(self):
      ...
      if len(self.towers[-1]) == self.size:
         return True
...
def main(gameSize):
   ...
   while True:
      ...
      # Default state of move for errors in move parsing
      move = []
